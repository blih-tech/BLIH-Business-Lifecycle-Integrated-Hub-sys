...
export function CandidateDetailDialog({
  ...
  const fallbackAnswers: OngoingPipelineCandidate['answers'] = [
    ...
    },
    {
      id: 'fallback-3',
      label: 'Full Name',
      value: 'Jessica David Parker',
      type: 'text',
    },
    {
      id: 'fallback-4',
      label: 'Email Address',
      value: 'applicant@gmail.com',
      type: 'text',
    },
    {
      id: 'fallback-5',
    ...
    { id: 'fallback-8', label: 'Notice Period', value: '7 Days', type: 'text' },
    {
      id: 'fallback-9',
      label: 'Portfolio',
      value: 'https://portfolio.example.com/applicant',
      type: 'link',
    },
    {
      id: 'fallback-10',
      label: 'Link to Portfolio',
      value: 'https://applicant-portfolio.com',
      type: 'link',
    },
    {
      id: 'fallback-11',
      label: 'Additional Link',
      value: 'https://linkedin.com/applicant',
      type: 'link',
    },
    {
      id: 'fallback-12',
      label: 'Resume / CV',
      value: 'https://files.example.com/Applicant CV.pdf',
      type: 'file',
    },
    {
      id: 'fallback-13',
  ...
  const answers = candidate?.answers?.length
    ? candidate.answers
    : fallbackAnswers;
  const fallbackValues = fallbackAnswers.reduce<Record<string, string>>(
    (acc, answer) => {
  ...
  const experience = getAnswerValue(
    answers,
  ...
  const linkToPortfolio = getAnswerValue(
    answers,
    'Link to Portfolio',
    fallbackValues,
  );
  const additionalLink = getAnswerValue(
    answers,
    'Additional Link',
    fallbackValues,
  );

  const whyJoin = getAnswerValue(
    answers,
    'Why Do You Want to Us?',
    fallbackValues,
  );
  const goodFit = getAnswerValue(
    answers,
  ...
  const matchScore =
    job?.topMatch?.matchScore ?? candidate?.aiAnalysis.score ?? 87;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] overflow-y-auto p-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:max-w-[900px]">
        <DialogHeader className="sr-only">
          ...
          <DialogDescription>
            Applicant details, documents, and actions.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-6 p-6">
          ...
          <div className="grid gap-6 border-b border-[#e5e5e5] pb-6 text-sm sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <p className="bg-[#f2f2f2] px-3 py-2 text-sm font-semibold text-black">
                {linkToPortfolio}
              </p>
            </div>
            <div className="space-y-2">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <Button
              variant="outline"
              className="h-11 w-full gap-2 text-primary"
            >
            ...
            <Button
              variant="outline"
              className="h-11 w-full border-red-500 text-red-500"
            >
