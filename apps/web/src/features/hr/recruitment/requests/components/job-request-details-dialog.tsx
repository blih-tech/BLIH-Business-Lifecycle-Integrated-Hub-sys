...
export function JobRequestDetailsDialog({ request, onOpenChange }: JobRequestDetailsDialogProps) {
  return (
          ...
          <>
            <DialogHeader className="border-b border-border p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <DialogTitle className="ui-section-title text-foreground">{request.title}</DialogTitle>
                <span
                  className={`inline-flex rounded-md px-2 py-0.5 text-xs font-medium ${priorityClass(request.priority)}`}
                >
                  {priorityLabel(request.priority)} Priority
                </span>
              </div>
              <DialogDescription className="mt-1 ui-body text-muted-foreground">
                Review detailed requisition information before you proceed with approval.
              </DialogDescription>
            ...
            <div className="grid gap-3 p-4 lg:grid-cols-[1.1fr_0.9fr]">
              <div className="space-y-3">
                <section className="ui-surface p-3.5">
                  <p className="ui-section-title text-foreground">Request Snapshot</p>
                  <div className="mt-2 grid grid-cols-1 gap-2.5 sm:grid-cols-2">
                    <DetailItem label="Requisition ID" value={requestIdLabel(request.id)} />
                    <DetailItem label="Department" value={departmentLabel(request.department)} />
                    <DetailItem label="Employment Type" value={request.employmentType} />
                    <DetailItem label="Open Positions" value={request.positions} />
                    <DetailItem label="Requested On" value={request.requestedAt} />
                    <DetailItem label="Expected Start" value={request.expectedStartDate} />
                  </div>
                </section>

                <section className="ui-surface p-3.5">
                  <p className="ui-section-title text-foreground">Request Owners</p>
                  <div className="mt-2 grid grid-cols-1 gap-2.5 sm:grid-cols-2">
                    <DetailItem label="Requested By" value={request.requestedBy} />
                    <DetailItem label="Hiring Manager" value={request.hiringManager} />
                    <DetailItem label="Experience Level" value={request.experienceLevel} className="sm:col-span-2" />
                  </div>
                </section>
              </div>
              ...
              <div className="space-y-3">
                <section className="ui-surface p-3.5">
                  <p className="ui-section-title text-foreground">Business Justification</p>
                  <p className="ui-body mt-2 text-muted-foreground">{request.justification}</p>
                </section>
                ...
                <section className="ui-surface p-3.5">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {request.keySkills.map((skill) => (
                      <span key={skill} className="rounded-md border border-border bg-muted px-2 py-1 text-xs">
                        {skill}
                      </span>
                    ))}
                  </div>
