...
function iconFor(icon: RequestsStatItem["icon"]) {
  ...
  if (icon === "approved") return <CircleCheckBig className="h-4 w-4 text-primary" />;
  ...
  return (
    <article className="ui-surface p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="inline-flex h-6 w-6 items-center justify-center">{iconFor(item.icon)}</span>
      </div>
    </article>
  );
}
