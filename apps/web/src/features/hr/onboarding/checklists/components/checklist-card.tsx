  ...
  return (
    <article className="rounded-[12px] border border-[#e5e5e5] bg-white p-5">
      <div className="space-y-1">
        <h3 className="text-lg font-medium tracking-[-0.3125px] text-black">
          {checklist.title}
        </h3>
      ...
      <div className="mt-4 rounded-[8px] bg-[#f5f5f5] px-3 pt-3 pb-2">
        ...
        <ul className="mt-1 space-y-1">
          {checklist.items.map((item) => (
            <li
              key={item}
              className={`text-sm tracking-[-0.1504px] ${item.startsWith('+') ? 'font-medium text-primary' : 'text-[#666]'}`}
            >
              {item.startsWith('+') ? (
                item
              ) : (
      ...
      <div className="mt-3.5 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            className="grid h-8 w-8 place-items-center rounded-[6px] border border-[#e5e5e5] bg-white text-[#666]"
          >
