...
export function PerformanceDistributionCard({ data }: PerformanceDistributionCardProps) {
  return (
    <Card className="gap-0 rounded-[10px] border-border py-0 shadow-none">
      <CardContent className="p-3 md:p-4">
        ...
        <ChartContainer config={chartConfig} className="mt-3 h-[180px] w-full">
          <PieChart>
            <Pie data={data} dataKey="value" nameKey="label" cx="50%" cy="48%" innerRadius={0} outerRadius={72} paddingAngle={0}>
              {data.map((entry) => (
