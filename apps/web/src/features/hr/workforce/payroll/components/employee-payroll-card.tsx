        ...
        <div className="flex items-start gap-2.5">
          ...
          <div className="min-w-0 space-y-0.5">
            <div className="flex items-center gap-2">
              <p className="truncate text-sm font-semibold tracking-[-0.176px] text-black">
                {item.name}
              </p>
...
function MetricCell({ metric }: { metric: PayrollMetric }) {
  ...
  return (
    <div
      className={
        metric.highlight
          ? 'rounded-[8px] bg-[#eff4ff] p-2'
          : 'rounded-[8px] bg-[#f7f7f7] p-2'
      }
    >
      <p className="text-[10px] leading-4 text-[#666]">{metric.label}</p>
      <p className={`text-sm font-semibold tracking-[-0.15px] ${toneClass}`}>
        {metric.value}
      </p>
    </div>
  );
