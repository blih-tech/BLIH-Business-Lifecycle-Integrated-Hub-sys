...
export function AnnualProfitSharingSection({
  summaries,
  tiers,
}: AnnualProfitSharingSectionProps) {
  return (
      ...
      <CardContent className="space-y-4 p-4">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {summaries.map((summary) => (
            <div
              key={summary.id}
              className="rounded-[12px] border border-[#e5e5e5] bg-white px-4 py-3 text-center"
            >
              <p className="text-xs text-[#666]">{summary.label}</p>
              <p className="mt-1 text-lg font-semibold tracking-[0.0703px] text-black">
                {summary.value}
              </p>
            </div>
          ))}
