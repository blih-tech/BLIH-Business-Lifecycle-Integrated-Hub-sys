...
export function MonthlyPayrollTrendCard({ data }: MonthlyPayrollTrendCardProps) {
  return (
    <Card className="gap-0 rounded-[12px] border-border py-0 shadow-none">
      <CardContent className="space-y-3 p-4">
        <p className="text-sm leading-4 tracking-[-0.3125px] text-black">Monthly Payroll Trend</p>
        <ChartContainer config={chartConfig} className="h-[250px] w-full">
          <LineChart data={data} margin={{ left: 8, right: 8, top: 6, bottom: 0 }}>
            ...
            <Line
              ...
              strokeWidth={2}
              dot={<Dot r={2.7} fill="var(--primary)" stroke="var(--primary)" />}
              activeDot={{ r: 4 }}
            />
          </LineChart>
        </ChartContainer>
