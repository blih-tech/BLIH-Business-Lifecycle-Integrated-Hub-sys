...
export function DepartmentBudgetUtilizationCard({ data }: DepartmentBudgetUtilizationCardProps) {
  return (
    <Card className="gap-0 rounded-[12px] border-border py-0 shadow-none">
      <CardContent className="space-y-3 p-4">
        <p className="text-sm leading-4 tracking-[-0.3125px] text-black">Department Budget Utilization</p>
        <ChartContainer config={chartConfig} className="h-[250px] w-full">
          <BarChart data={data} margin={{ left: 8, right: 8, top: 6, bottom: 0 }} barCategoryGap={12}>
