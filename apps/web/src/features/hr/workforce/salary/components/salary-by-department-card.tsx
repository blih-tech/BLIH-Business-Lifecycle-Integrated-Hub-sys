...
export function SalaryByDepartmentCard({
  data,
  summaries,
}: SalaryByDepartmentCardProps) {
  return (
    <Card className="gap-0 rounded-[12px] border-border py-0 shadow-none">
      <CardContent className="space-y-4 p-4">
        ...
        <ChartContainer config={chartConfig} className="h-[142px] w-full">
          <BarChart
            data={data}
            margin={{ left: 10, right: 10, top: 8, bottom: 2 }}
          >
        ...
        <div className="grid gap-2 md:grid-cols-3">
          {summaries.map((item) => (
            <div key={item.id} className="rounded-[8px] bg-[#f3f3f3] px-3 py-3">
              ...
              <p className="text-[14px] font-medium leading-6 text-primary">
                {item.averageSalary}
              </p>
