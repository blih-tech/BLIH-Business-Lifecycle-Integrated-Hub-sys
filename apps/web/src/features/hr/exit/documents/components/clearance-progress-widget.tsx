...
export function ClearanceProgressWidget({ progressText, progressTasks }: ClearanceProgressWidgetProps) {
  const numericProgress = Number.parseFloat(progressText.replace("%", ""));
  const clampedProgress = Number.isFinite(numericProgress) ? Math.max(0, Math.min(numericProgress, 100)) : 100;
    ...
    <div className="relative h-full w-[124px] rounded-[8px] border border-primary bg-[rgba(30,102,247,0.1)] p-2">
      ...
      <div className="mt-1 flex justify-center">
        <div className="relative grid h-[89px] w-[89px] place-items-center">
          ...
          <span className="absolute text-[20px] font-bold leading-none tracking-[0.3955px] text-primary">{progressText}</span>
        </div>
      </div>
      ...
      <span className="absolute right-[8px] top-[8px] text-[14px] leading-none text-[#ffe345]">✦</span>
    </div>
  );
}
