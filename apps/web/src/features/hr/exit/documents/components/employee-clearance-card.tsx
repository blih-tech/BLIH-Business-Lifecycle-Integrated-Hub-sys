      ...
      <CardContent className="space-y-3 p-4">
        <div className="grid gap-3 xl:grid-cols-[1fr_124px]">
          <div className="space-y-3">
            ...
            <p className="text-sm text-[#666]">Last Working Day: {item.lastWorkingDay}</p>
            <div className="space-y-2 border-t border-[#e5e5e5] pt-3">
              <DocumentsRow items={firstRow} />
              <DocumentsRow items={secondRow} />
            </div>
          </div>
          <ClearanceProgressWidget progressText={item.progressText} progressTasks={item.progressTasks} />
        </div>
...
function DocumentsRow({ items }: { items: { id: string; label: string }[] }) {
  return (
    <div className="flex flex-wrap gap-2">
      {items.map((item) => (
        <div key={item.id} className="inline-flex h-5 items-center rounded-[6px] bg-[#dbe6fb] px-2 text-xs text-black">
