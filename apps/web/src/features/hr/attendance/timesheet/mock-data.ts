...
function buildRows(seed: string) {
  ...
  return employees.map((employee, index) => {
    ...
    const metrics =
      status === "on-track" ? onTrackMetrics : status === "overtime" ? overtimeMetrics : onLeaveMetrics;
...
export const attendanceTimesheetStats: AttendanceTimesheetStatItem[] = [
  { id: "avg-month", label: "Avg Work Hours This Month", value: "12", icon: "clock-3" },
  { id: "avg-week", label: "Avg Work Hours This Week", value: "28", icon: "clock-3" },
...
export const timesheetDepartments = ["all", "Marketing", "Finance", "Technology"] as const;

export const timesheetStatusOptions = ["all", "on-track", "overtime", "on-leave"] as const;
