...
import type { PerformanceCardItem } from '@/features/hr/attendance/overview/types';

type PerformanceCardProps = {
  stat: PerformanceCardItem;
};

function PerformanceIcon({ icon }: { icon: PerformanceCardItem['icon'] }) {
...
export function PerformanceCard({ stat }: PerformanceCardProps) {
  return (
    <article className="rounded-[8px] bg-[#f3f3f3] p-4">
      <div className="flex items-start justify-between">
        ...
        <PerformanceIcon icon={stat.icon} />
      </div>

      <p className="mt-1 text-[34px] font-semibold leading-8 tracking-[0.0703px] text-primary">
        {stat.value}
      </p>
