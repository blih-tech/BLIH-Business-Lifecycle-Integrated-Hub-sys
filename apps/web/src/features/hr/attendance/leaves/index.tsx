...
export function AttendanceRequestsContent() {
  ...
  const [selectedRequestId, setSelectedRequestId] = useState<string | null>(
    previousLeaveRows[0]?.id ?? null,
  );

  const filteredRows = useMemo(() => {
    const query = searchTerm.trim().toLowerCase();
    if (!query) return previousLeaveRows;
    return previousLeaveRows.filter((row) => {
      return (
  ...
  return (
    <main className="mx-auto w-full max-w-[1024px] space-y-10 px-4 py-4 md:px-5 md:py-5">
      <StatsGrid items={attendanceRequestStats} />
