...
const generatedEmployees: DirectoryEmployee[] = Array.from(
  { length: 24 },
  (_, index) => {
    const template =
      baseDirectoryEmployees[(index + 1) % baseDirectoryEmployees.length]!;
    const numericId = index + 9;
    const suffix =
      nameSuffixes[Math.floor(index / baseDirectoryEmployees.length)] ?? 'V';
    const emailLocalPart = template.name
      .toLowerCase()
      .replace(/\s+/g, '.')
      .replace(/[^a-z.]/g, '');
