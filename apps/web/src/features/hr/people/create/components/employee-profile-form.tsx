...
import { employeeProfileFormSchema, type EmployeeProfileFormValues } from "@/features/hr/people/create/form-schema";
...
type EmployeeProfileFormProps = {
  onCancel: () => void;
  onSuccess: (values: EmployeeProfileFormValues, action: "create" | "save-draft") => void;
};

export function EmployeeProfileForm({ onCancel, onSuccess }: EmployeeProfileFormProps) {
  const [submitAction, setSubmitAction] = useState<"create" | "save-draft">("create");
  ...
  return (
    <section className="rounded-[12px] border border-[#e5e5e5] bg-white p-5 md:p-6">
      <h2 className="text-lg font-semibold tracking-[-0.3125px] text-black">Employee Profile Form</h2>
      <p className="mt-1 text-sm tracking-[-0.1504px] text-[#666]">Fill all required details before creating a profile.</p>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="mt-5 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            ...
            >
              {form.formState.isSubmitting && submitAction === "create" ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
              Create
            </Button>
            <Button
              type="submit"
