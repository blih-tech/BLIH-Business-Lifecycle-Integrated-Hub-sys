      ...
      <CardContent className="space-y-3 p-3">
        ...
        {item.progress !== undefined ? (
          <div>
            <div className="mb-1 flex items-center justify-between">
              ...
              <p className="text-[11px] font-semibold text-primary">{item.progress}%</p>
            </div>
            <div className="h-[6px] w-full rounded-full bg-[#dbe6fb]">
              <div className="h-full rounded-full bg-primary" style={{ width: `${item.progress}%` }} />
            </div>
            {item.dueDate ? <p className="mt-1 text-[10px] text-[#666]">Due: {item.dueDate}</p> : null}
          </div>
        ) : null}

        <div className="rounded-[4px] bg-[#f3f3f3] p-2">
          <p className="mb-1 text-[10px] font-medium text-black">Identified Skill Gaps</p>
          <div className="flex flex-wrap gap-1">
            {item.identifiedGaps.map((gap) => (
              <span key={gap} className="rounded-[4px] border border-primary/40 bg-white px-1.5 py-0.5 text-[9px] text-primary">
                {gap}
              </span>
            ))}
          </div>
        ...
        {item.recommendedActions?.length ? (
          <div className="rounded-[4px] bg-[#dbe6fb] p-2">
            <p className="mb-1 text-[10px] font-medium text-black">Recommended Actions</p>
