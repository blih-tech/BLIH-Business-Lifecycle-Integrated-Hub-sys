...
export class VerifyProfileUseCase {
  ...
  async execute(
    ...
    if (checklist.taskInstance.targetDataModel !== 'USER_PROFILE') {
      throw new BadRequestException('Task does not map to USER_PROFILE');
    }
    ...
    await this.prisma.$transaction(async (tx) => {
      if (dto.approved) {
        // Mark UserProfile as VERIFIED (now has the status field)
        ...
        // Reject — put back to REJECTED with feedback
        await tx.userProfile.update({
          where: { employeeId },
