...
function canScheduleInterview(interviewerIds, proposedTime, duration) {
  ...
  for (const interviewerId of interviewerIds) {
    ...
    if (conflicts.has_tentative)
      return { canSchedule: true, warning: 'TENTATIVE_CONFLICT' };
    if (conflicts.outside_work_hours)
      return { canSchedule: true, warning: 'OVERTIME' };
  }
...
function checkProbationStatus(employee) {
  ...
  if (daysUntilEnd === 7) {
    notify(
      'MANAGER',
      `URGENT: Probation review due in 7 days for ${employee.name}`,
    );
  }
...
function checkDocumentExpirations() {
  ...
  for (const doc of documents) {
    ...
    if (daysUntilExpiry === 7) {
      ...
      notify(
        'HR',
        `URGENT: ${doc.employee_name}'s ${doc.type} expires in 7 days.`,
      );
      notify(
        'MANAGER',
        `${doc.employee_name}'s document expiring soon - may affect work.`,
      );
    }
    ...
    if (daysUntilExpiry <= 0) {
      ...
      if (doc.is_mandatory) {
        notify(
          'HR',
          `MANDATORY DOCUMENT EXPIRED: ${doc.employee_name} - ${doc.type}`,
        );
...
function calculateLeaveBalance(employee, leaveType, asOfDate) {
  ...
  // Get used leave
  const usedLeave = db.leave_requests
    .aggregate({
  ...
  const pendingLeave = db.leave_requests
    .aggregate({
      employee_id: employee.id,
      leave_type: leaveType,
      status: 'APPROVED',
  ...
  return {
    ...
    available:
      Math.round((entitledToDate - usedLeave - pendingLeave) * 10) / 10,
...
function validateLeaveRequest(request) {
  ...
  if (overlapBlackout) {
    errors.push({
      field: 'dates',
      message:
        'Request conflicts with blackout period (critical project/quarter end)',
...
if (
  ...
  isIpInOfficeRange(clientIP, config.ipCidrs)
) {
...
function calculateReviewWindow(period) {
  ...
  return {
    window_opens: subDays(
      endOfPeriod,
      reviewSchedule.quarterly.window_open_days_before,
    ),
    self_assessment_due: addDays(
      endOfPeriod,
      reviewSchedule.quarterly.self_assessment_duration_days,
    ),
    manager_review_due: addDays(
      endOfPeriod,
      reviewSchedule.quarterly.self_assessment_duration_days +
        reviewSchedule.quarterly.manager_review_duration_days,
    ),
    window_closes: addDays(
      endOfPeriod,
      reviewSchedule.quarterly.window_close_days_after,
    ),
  };
}
```
...
function calculateFinalRating(employeeId, reviewId) {
  ...
  const selfRatings = review.self_assessment.goal_ratings.map(
    (g) => g.self_rating,
  );
  const selfAverage = average(selfRatings);

  // Manager ratings
  const managerRatings = review.manager_review.goal_ratings.map(
    (g) => g.manager_rating,
  );
  ...
  return {
    ...
    promotion_eligible:
      category === 'EXCEEDS_EXPECTATIONS' || category === 'OUTSTANDING',
  };
}
```
...
function calculateOKRProgress(okrId) {
  ...
  const achievedKRs = krProgress.filter(
...
function calculateTrainingBudget(teamId, year) {
  ...
  return {
    ...
    remaining: Math.round(
      baseAllocation * multiplier - getUsedBudgetYTD(teamId, year),
    ),
  };
}
```
...
function analyzeSkillGap(employeeId, targetRoleId) {
  ...
  for (const requiredSkill of targetSkills) {
    const currentLevel =
      currentSkills.find((s) => s.skill_id === requiredSkill.skill_id)?.level ||
      0;
...
function determineDisciplinaryAction(employeeId, incidentType) {
  // Check history
  const history = db.disciplinary_actions
    .find({
  ...
  const activeCount = history.filter(
    (h) => !h.expires_at || h.expires_at > new Date(),
  ).length;
...
function processIncidentReport(incidentId) {
  ...
  // Auto-notify
  notify(
    incident.investigator_id,
    `New incident assigned: ${incident.report_id}`,
  );
...
function calculateFinalSettlement(employeeId, lastWorkingDay) {
  ...
  settlement.earnings.salary_days_worked = Math.round(
    dailyRate * daysWorkedInFinalMonth,
  );
  ...
  if (leaveBalance.available > 0) {
    settlement.earnings.leave_encashment = Math.round(
      dailyRate * leaveBalance.available,
    );
  }
  ...
  const grossEarnings = Object.values(settlement.earnings).reduce(
    (a, b) => a + b,
    0,
  );
  ...
  if (outstanding > 0) {
    settlement.deductions.loan_recovery = Math.min(
      outstanding,
      grossEarnings * 0.5,
    ); // Max 50% recovery
  }
  ...
  // Calculate net
  const totalDeductions = Object.values(settlement.deductions).reduce(
    (a, b) => a + b,
    0,
  );
...
const offboardingTasks = {
  HR: [
    ...
    {
      task: 'Prepare experience certificate',
      deadline_days: 0,
      mandatory: true,
    },
    {
      task: 'Update employee status to RESIGNED',
      deadline_days: 0,
      mandatory: true,
    },
  ...
  MANAGER: [
    ...
    {
      task: 'Provide final feedback to HR',
      deadline_days: -1,
      mandatory: false,
    },
  ],
...
function escalateApproval(request, currentLevel) {
  const nextLevel = request.approval_chain.find(
  ...
  if (nextLevel) {
    ...
    notify(
      'HR_ADMIN',
      `Approval stuck at ${currentLevel.role} for ${request.type}`,
    );
  }
...
function processParallelApprovals(request) {
  const pendingApprovers = request.approval_chain.filter(
    (l) => l.status === 'PENDING',
  );
...
_For: BLIH Backend Development Team_
